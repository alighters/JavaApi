...
public interface IPostService {
     ...
     * @param maxId   若指定此参数，则只返回ID比max_id小的动态更新（即比max_id发表时间早的动态更新）
     * @param pagesize   指定要返回的记录条数(int默认值20，最大值100)
     * @param selectType   查询类型 （2：图片,3:文档，4：问答）
     **/
     ...
     *
     * @param accessToken   当前登录用户访问令牌
     * @param postId   回复的动态更新编号
     * @param replyId   回复编号（可以对别人的回复进行回复）[可选]
     * @param replyMsg   回复的消息内容([aid]accountID[/aid]代表@某个人,[gid]groupID[/gid]代表@某个群组)
     * @param fileType   可为空(为空时 p_img或p_doc也必须为空)，picture：表示上传图片；document：表示上传文档
     * @param pImg或pDoc   要上传的图片、文档。图片仅支持JPEG,GIF,PNG,目前上传图片大小限制为<8M。文档仅支持DOC,PDF,XLS,PPT,TXT,压缩包,目前上传文件大小限制为<50M
     ...
     *
     * @param accessToken   当前登录用户访问令牌
     * @param postId   动态更新编号(post_id为空 只创建个标签)
     * @param tag   标签名称
     * @param chooseType   操作类型 添加=true/删除=false
     **/
     ...
     * @param accessToken   当前登录用户访问令牌
     * @param postId   动态更新编号
     * @param chooseType   操作类型 喜欢=1/收藏=2
     **/
     ...
     * @param accessToken   当前登录用户访问令牌
     * @param groupIds   可为空，动态分享群组编号(多个群组用逗号隔开)
     * @param postMsg   动态更新内容(###userID###代表@某个人,$$$groupID$$$代表@某个群组，#标签内容#代表给动态定义标签)
     * @param postType   动态更新类型(0表示普通动态更新（默认值）1表示链接动态更新4表示问答动态更新)
     * @param shareType   分享范围(0表示分享给所有同事;1表示群内分享；2表示所有关注者和群组；3表示分享给自己； 默认0表示分享给所有同事)
     **/
